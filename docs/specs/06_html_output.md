...
- 合計は `スコア N/12`。PC ではスコアパネル上部（`.score-total`）、モバイルではメタ行（`.score-inline`）に出す
- 各軸（新規性 / 関心の近さ / 読む必要性 / 活用度）は `.bar` > `.fill.sN` で描画し、`N`（0〜3）に応じて CSS で幅を決める（s0 は非表示、s1=33% / s2=67% / s3=100%）
- `scores` を持たない旧データではスコアパネル・`.score-inline` ともに出力しない
...
- レスポンシブ（ブレークポイントは 2 つ）
  - `max-width: 680px`: カードを 1 カラムに切り替え、スコアパネルは要約の下へ回す。合計スコアはメタ行の `.score-inline` に移す。4 軸は `auto-fit` グリッドで 2 列 → さらに狭ければ 1 列。ヘッダーの統計は非表示
